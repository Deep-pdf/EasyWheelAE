...
fn main() {
    ...
    let mut state = State { all_windows: Vec::new() };
    unsafe {
    ...
    for (hwnd, class_str, title_str) in &state.all_windows {
        if class_str.contains("Chrome") || title_str.contains("Chrome") || title_str.contains("Google") {
            println!("  HWND: {:?}, Class: '{}', Title: '{}'", hwnd, class_str, title_str);
        }
    }
